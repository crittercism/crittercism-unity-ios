...
void CreateSplashView( UIView* parentView )
{
    ...
    NSString* launchImageName = [NSString stringWithFormat:@"Default%s%s", orientSuffix, szSuffix];
	...
	_splashPortrait = needPortraitSplash;
...
-(BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
	...
	if(_splashView)
	{
		if(_splashShowing)
		{
			bool requestPortrait = (targetOrient == portrait || targetOrient == portraitUpsideDown);
